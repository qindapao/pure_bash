...
struct_del_field ()
{
    ...
    # 不是顶级键的情况,从上到下查找
    eval local _struct_del_field_data_lev{1..20}=''
    ...
    for _struct_del_field_index in "${@:1:$#}" ; do
        ...
        local -n _struct_del_field_data_lev_ref=_struct_del_field_data_lev${_struct_del_field_lev_cnt}

        # 转换上一个数据类型
        if [[ "$_struct_del_field_data_lev_ref_last" =~ ^(declare)\ ([^\ ]+)\ _struct_set_field_data_lev[0-9]+=(.*) ]] ; then
            declare ${BASH_REMATCH[2]} _struct_del_field_data_lev${_struct_del_field_lev_cnt}
